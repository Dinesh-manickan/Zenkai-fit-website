  ...
  return (
    <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
      <div className="flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-x-4">
        <h4 className="text-3xl hidden sm:inline sm:text-slate-400">
          0{i + 1}
        </h4>
      ...
      <div className="flex flex-col cursor-pointer rounded bg-slate-900">
        <button
          onClick={handleClick}
          className="flex items-center justify-between p-2 w-full"
        >
          <p className="font-medium">- Description</p>
          <i className={`fa-solid ${isClicked ? "fa-minus" : "fa-plus"}`}></i>
        </button>

        {isClicked && (
          <>
            <div className="h-[1px] bg-slate-950 gap-1"></div>
            <ul className="flex flex-col text-sm p-2 gap-1 list-disc list-inside">
